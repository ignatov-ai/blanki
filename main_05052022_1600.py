...
new_size = (int(614*1.1),int(857*1.1))
...
# подсчет количества черных пикселей и процентного соотношения черных пикселей к белым
...
for i in range(0,4):
    ...
    center = ((width*((i)%2) + height*((i+1)%2)) // 2, (width*((i+1)%2) + height*((i)%2)) // 2)
    print(center, width, height)

    M = cv2.getRotationMatrix2D(center, (90*i) % 360, 1.0)
    rotated = cv2.warpAffine( scanBW, M, (width*((i)%2) + height*((i+1)%2), width*((i+1)%2) + height*((i)%2)) )
    dx,dy = 5,5
    x,y = width,40
    ...
    crop_img = rotated[height - y:height-dy, dx:width-dx]
    ...
    #print(s, count, s-count, blackProcent, (90*i) % 360 )
    print(blackProcent)
